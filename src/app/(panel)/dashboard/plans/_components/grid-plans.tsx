  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 max-w-5xl mx-auto py-8">
      {subscriptionPlans.map((plan, index) => (
        ...
        >
          {index === 1 && (
            <div className="bg-blue-500 w-full py-2 text-center rounded-t-lg">
          ...
          <CardHeader className={index !== 1 ? "pt-8" : ""}>
            ...
            <CardDescription className="text-slate-500 mt-2 min-h-[48px]">
              {plan.description}
            </CardDescription>
          </CardHeader>
                ...
                <li key={featureIndex} className="flex items-start gap-3">
                  <Check
                    className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                      index === 1 ? "text-blue-500" : "text-slate-400"
                    }`}
                  />
